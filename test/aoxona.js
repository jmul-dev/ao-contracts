...
contract("AOXona", function(accounts) {
	...
	contract("Network Ion Function Tests", function() {
		...
		it("buy() - user can buy network ions", async function() {
			...
			await aoxona.mint(aoxona.address, 10 ** 20, { from: whitelistedAddress });
		...
		it("The AO - transferETH() should be able to transfer ETH to an address", async function() {
			await aoxona.buy({ from: account2, value: web3.toWei(2, "ether") });

			var canTransferEth;
			try {
				await aoxona.transferEth(recipient.address, web3.toWei(1, "ether"), { from: someAddress });
				canTransferEth = true;
			} catch (e) {
			...
			try {
				await aoxona.transferEth(emptyAddress, web3.toWei(1, "ether"), { from: theAO });
				canTransferEth = true;
			} catch (e) {
				canTransferEth = false;
			}
			...
			try {
				await aoxona.transferEth(recipient.address, web3.toWei(1000, "ether"), { from: theAO });
				canTransferEth = true;
			} catch (e) {
				canTransferEth = false;
			}
			...
			try {
				await aoxona.transferEth(recipient.address, web3.toWei(1, "ether"), { from: theAO });
				canTransferEth = true;
			} catch (e) {
				canTransferEth = false;
			}
