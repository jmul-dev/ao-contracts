...
contract("AOXona", function(accounts) {
	...
	contract("Network Token Function Tests", function() {
		...
		it("Whitelisted address - mintToken()  can mint token", async function() {
			var canMint;
			try {
				await aoxona.mintToken(account1, 100, { from: someAddress });
				canMint = true;
			} catch (e) {
				canMint = false;
			}
			...
			try {
				await aoxona.mintToken(account1, 100, { from: whitelistedAddress });
				canMint = true;
			} catch (e) {
				canMint = false;
			}
		...
		it("Whitelisted address - mintTokenEscrow() should be able to mint and escrow tokens to an account", async function() {
			...
			var canMintEscrow;
			try {
				await aoxona.mintTokenEscrow(account1, 10, { from: someAddress });
				canMintEscrow = true;
			} catch (e) {
				canMintEscrow = false;
			}
			...
			try {
				await aoxona.mintTokenEscrow(account1, 10, { from: whitelistedAddress });
				canMintEscrow = true;
			} catch (e) {
				canMintEscrow = false;
			}
		...
		it("buy() - user can buy network tokens", async function() {
			...
			await aoxona.mintToken(aoxona.address, 10 ** 20, { from: whitelistedAddress });
