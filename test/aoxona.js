...
contract("AOXona", function(accounts) {
	...
	contract("Variable settings", function() {
		it("should return correct name", async function() {
			var name = await aoxona.name();
			assert.equal(name, "AO Xona", "Contract has the incorrect name");
		});

		it("should return correct symbol", async function() {
			var symbol = await aoxona.symbol();
			assert.equal(symbol, "AOXONA", "Contract has the incorrect symbol");
		});

		it("should have the correct power of ten", async function() {
			var powerOfTen = await aoxona.powerOfTen();
			assert.equal(powerOfTen, 27, "Contract has the incorrect power of ten");
		});

		it("should have 0 decimal", async function() {
			var decimals = await aoxona.decimals();
			assert.equal(decimals, 27, "Contract has the incorrect decimals");
		});

		it("should have 0 initial supply", async function() {
			var totalSupply = await aoxona.totalSupply();
	...
	contract("The AO Only", function() {
		...
		it("The AO - transferOwnership() - should be able to transfer ownership to a TAO", async function() {
			...
			try {
				await aoxona.transferOwnership(taoId, { from: someAddress });
			...
			try {
				await aoxona.transferOwnership(taoId, { from: theAO });
				canTransferOwnership = true;
			} catch (e) {
				canTransferOwnership = false;
			}
			...
			var newTheAO = await aoxona.theAO();
		...
		it("The AO - setWhitelist() should be able to whitelist an address", async function() {
			var canSetWhitelist;
			try {
				await aoxona.setWhitelist(whitelistedAddress, true, { from: someAddress });
			...
			try {
				await aoxona.setWhitelist(whitelistedAddress, true, { from: account1 });
				canSetWhitelist = true;
			} catch (e) {
				canSetWhitelist = false;
			}
			...
			var whitelistStatus = await aoxona.whitelist(whitelistedAddress);
		...
		it("The AO - setNameTAOPositionAddress() should be able to set NameTAOPosition address", async function() {
			var canSetAddress;
			try {
				await aoxona.setNameTAOPositionAddress(nametaoposition.address, { from: someAddress });
				canSetAddress = true;
			} catch (e) {
				canSetAddress = false;
			}
			...
			try {
				await aoxona.setNameTAOPositionAddress(nametaoposition.address, { from: account1 });
				canSetAddress = true;
			} catch (e) {
				canSetAddress = false;
			}
		...
		it("The AO - freezeAccount() can freeze account", async function() {
			...
			try {
				await aoxona.freezeAccount(account2, true, { from: someAddress });
			...
			try {
				await aoxona.freezeAccount(account2, true, { from: account1 });
				canFreezeAccount = true;
			} catch (e) {
				canFreezeAccount = false;
			}
			assert.equal(canFreezeAccount, true, "The AO can't mint AOXona");
			var account2Frozen = await aoxona.frozenAccount(account2);
			assert.equal(account2Frozen, true, "Account2 is not frozen after The AO froze his account");

			await aoxona.freezeAccount(account2, false, { from: account1 });
		});

		it("The AO - setPrices() can set prices", async function() {
			var canSetPrices;
			try {
				await aoxona.setPrices(2, 2, { from: someAddress });
				canSetPrices = true;
			} catch (e) {
				canSetPrices = false;
			}
			assert.notEqual(canSetPrices, true, "Others can set network AOXona prices");
			try {
				await aoxona.setPrices(2, 2, { from: account1 });
				canSetPrices = true;
			} catch (e) {
				canSetPrices = false;
			}
			assert.equal(canSetPrices, true, "The AO can't set network AOXona prices");
			var sellPrice = await aoxona.sellPrice();
			var buyPrice = await aoxona.buyPrice();
	...
	contract("Network Ion Function Tests", function() {
		...
		it("Whitelisted address - mint()  can mint", async function() {
			var canMint;
			try {
				await aoxona.mint(account1, 100, { from: someAddress });
				canMint = true;
			} catch (e) {
				canMint = false;
			}
			...
			var balanceBefore = await aoxona.balanceOf(account1);
			var totalSupplyBefore = await aoxona.totalSupply();
			try {
				await aoxona.mint(account1, 100, { from: whitelistedAddress });
				canMint = true;
			} catch (e) {
				canMint = false;
			}
			...
			var balanceAfter = await aoxona.balanceOf(account1);
			var totalSupplyAfter = await aoxona.totalSupply();
		...
		it("WhitelistedAddress - stakeFrom() should be able to stake ions on behalf of others", async function() {
			...
			var canStake;
			try {
				await aoxona.stakeFrom(account1, 10, { from: someAddress });
				canStake = true;
			} catch (e) {
				canStake = false;
			}
			...
			try {
				await aoxona.stakeFrom(account1, 100000, { from: whitelistedAddress });
				canStake = true;
			} catch (e) {
				canStake = false;
			}
			...
			try {
				await aoxona.stakeFrom(account1, 10, { from: whitelistedAddress });
				canStake = true;
			} catch (e) {
				canStake = false;
			}
		...
		it("Whitelisted address - unstakeFrom() should be able to unstake ions on behalf of others", async function() {
			...
			var canUnstake;
			try {
				await aoxona.unstakeFrom(account1, 10, { from: someAddress });
				canUnstake = true;
			} catch (e) {
				canUnstake = false;
			}
			...
			try {
				await aoxona.unstakeFrom(account1, 100000, { from: whitelistedAddress });
				canUnstake = true;
			} catch (e) {
				canUnstake = false;
			}
			...
			try {
				await aoxona.unstakeFrom(account1, 10, { from: whitelistedAddress });
				canUnstake = true;
			} catch (e) {
				canUnstake = false;
			}
		...
		it("Whitelisted address - escrowFrom() should be able to escrow ions on behalf of others", async function() {
			var account1BalanceBefore = await aoxona.balanceOf(account1);
			var account2BalanceBefore = await aoxona.balanceOf(account2);
			var account2EscrowedBalanceBefore = await aoxona.escrowedBalance(account2);
			var totalSupplyBefore = await aoxona.totalSupply();

			var canEscrow;
			try {
				await aoxona.escrowFrom(account1, account2, 10, { from: someAddress });
				canEscrow = true;
			} catch (e) {
				canEscrow = false;
			}
			...
			try {
				await aoxona.escrowFrom(account1, account2, 1000, { from: whitelistedAddress });
				canEscrow = true;
			} catch (e) {
				canEscrow = false;
			}
			...
			try {
				await aoxona.escrowFrom(account1, account2, 10, { from: whitelistedAddress });
				canEscrow = true;
			} catch (e) {
				canEscrow = false;
			}
			...
			var account1BalanceAfter = await aoxona.balanceOf(account1);
			var account2BalanceAfter = await aoxona.balanceOf(account2);
			var account2EscrowedBalanceAfter = await aoxona.escrowedBalance(account2);
			var totalSupplyAfter = await aoxona.totalSupply();
		...
		it("Whitelisted address - mintEscrow() should be able to mint and escrow ions to an account", async function() {
			var account1BalanceBefore = await aoxona.balanceOf(account1);
			var account1EscrowedBalanceBefore = await aoxona.escrowedBalance(account1);
			var totalSupplyBefore = await aoxona.totalSupply();

			var canMintEscrow;
			try {
				await aoxona.mintEscrow(account1, 10, { from: someAddress });
				canMintEscrow = true;
			} catch (e) {
				canMintEscrow = false;
			}
			...
			try {
				await aoxona.mintEscrow(account1, 10, { from: whitelistedAddress });
				canMintEscrow = true;
			} catch (e) {
				canMintEscrow = false;
			}
			...
			var account1BalanceAfter = await aoxona.balanceOf(account1);
			var account1EscrowedBalanceAfter = await aoxona.escrowedBalance(account1);
			var totalSupplyAfter = await aoxona.totalSupply();
		...
		it("Whitelisted address - unescrowFrom() should be able to unescrow ions for an account", async function() {
			...
			var canUnescrow;
			try {
				await aoxona.unescrowFrom(account1, 10, { from: someAddress });
				canUnescrow = true;
			} catch (e) {
				canUnescrow = false;
			}
			...
			try {
				await aoxona.unescrowFrom(account1, 100000, { from: whitelistedAddress });
				canUnescrow = true;
			} catch (e) {
				canUnescrow = false;
			}
			...
			try {
				await aoxona.unescrowFrom(account1, 10, { from: whitelistedAddress });
				canUnescrow = true;
			} catch (e) {
				canUnescrow = false;
			}
		...
		it("Whitelisted address - whitelistBurnFrom() should be able to burn ions on behalf of others", async function() {
			...
			var canBurn;
			try {
				await aoxona.whitelistBurnFrom(account1, 10, { from: someAddress });
				canBurn = true;
			} catch (e) {
				canBurn = false;
			}
			...
			try {
				await aoxona.whitelistBurnFrom(account1, 1000000, { from: whitelistedAddress });
				canBurn = true;
			} catch (e) {
				canBurn = false;
			}
			...
			try {
				await aoxona.whitelistBurnFrom(account1, 10, { from: whitelistedAddress });
				canBurn = true;
			} catch (e) {
				canBurn = false;
			}
		...
		it("Whitelisted address - whitelistTransferFrom() should be able to transfer ions from an address to another address", async function() {
			...
			var canTransferFrom;
			try {
				await aoxona.whitelistTransferFrom(account1, account2, 10, { from: someAddress });
			...
			try {
				await aoxona.whitelistTransferFrom(account1, account2, 1000000, { from: whitelistedAddress });
				canTransferFrom = true;
			} catch (e) {
				canTransferFrom = false;
			}
			...
			try {
				await aoxona.whitelistTransferFrom(account1, account2, 10, { from: whitelistedAddress });
				canTransferFrom = true;
			} catch (e) {
				canTransferFrom = false;
			}
		...
		it("buy() - user can buy network ions", async function() {
			await aoxona.setPrices(1, 1, { from: theAO });

			var canBuy;
			try {
				await aoxona.buy({ from: account2, value: 10 });
				canBuy = true;
			} catch (e) {
				canBuy = false;
			}
			assert.notEqual(canBuy, true, "Contract does not have enough network ion balance to complete user's ion purchase");
			await aoxona.mint(aoxona.address, 10 ** 10, { from: whitelistedAddress });
		...
		it("sell() - user can sell network ions to contract", async function() {
			await aoxona.setPrices(100, 1, { from: theAO });

			var canSell;
			try {
				await aoxona.sell(10, { from: account2 });
				canSell = true;
			} catch (e) {
				canSell = false;
			}
			...
			var account2BalanceBefore = await aoxona.balanceOf(account2);
			var contractBalanceBefore = await aoxona.balanceOf(aoxona.address);

			try {
				await aoxona.sell(5, { from: account2 });
				canSell = true;
			} catch (e) {
				canSell = false;
			}
			...
			var account2BalanceAfter = await aoxona.balanceOf(account2);
			var contractBalanceAfter = await aoxona.balanceOf(aoxona.address);
			assert.equal(
				account2BalanceAfter.toNumber(),
		...
		it("transfer() - should send correct `_value` to `_to` from your account", async function() {
			var account1BalanceBefore = await aoxona.balanceOf(account1);
			var account2BalanceBefore = await aoxona.balanceOf(account2);
			await aoxona.transfer(account2, 10, { from: account1 });
			account1BalanceAfter = await aoxona.balanceOf(account1);
			account2BalanceAfter = await aoxona.balanceOf(account2);
			assert.equal(
		...
		it("burn() - should remove `_value` ions from the system irreversibly", async function() {
			var account1BalanceBefore = await aoxona.balanceOf(account1);
			await aoxona.burn(10, { from: account1 });
			account1BalanceAfter = await aoxona.balanceOf(account1);
			assert.equal(
				account1BalanceAfter.toNumber(),
				account1BalanceBefore.sub(new BN(10)).toNumber(),
		...
		it("approve() - should set allowance for other address", async function() {
			var account2AllowanceBefore = await aoxona.allowance(account1, account2);
			await aoxona.approve(account2, 10, { from: account1 });
			var account2AllowanceAfter = await aoxona.allowance(account1, account2);
			assert.equal(
		...
		it("transferFrom() - should send `_value` ions to `_to` in behalf of `_from`", async function() {
			var canTransferFrom;
			try {
				await aoxona.transferFrom(account1, account2, 5, { from: someAddress });
				canTransferFrom = true;
			} catch (e) {
				canTransferFrom = false;
			}
			...
			try {
				await aoxona.transferFrom(account1, account2, 1000, { from: account2 });
				canTransferFrom = true;
			} catch (e) {
				canTransferFrom = false;
			}
			...
			try {
				await aoxona.transferFrom(account1, account2, 5, { from: account2 });
				canTransferFrom = true;
			} catch (e) {
				canTransferFrom = false;
			}
		...
		it("burnFrom() - should remove `_value` ions from the system irreversibly on behalf of `_from`", async function() {
			var canBurnFrom;
			try {
				await aoxona.burnFrom(account1, 5, { from: someAddress });
				canBurnFrom = true;
			} catch (e) {
				canBurnFrom = false;
			}
			...
			try {
				await aoxona.burnFrom(account1, 10, { from: account2 });
				canBurnFrom = true;
			} catch (e) {
				canBurnFrom = false;
			}
			...
			try {
				await aoxona.burnFrom(account1, 5, { from: account2 });
				canBurnFrom = true;
			} catch (e) {
				canBurnFrom = false;
			}
		...
		it("frozen account should NOT be able to transfer", async function() {
			await aoxona.freezeAccount(account1, true, { from: theAO });
		...
		it("transferBetweenPublicKeys() - should be able to transfer ions between public keys in a Name", async function() {
			...
			try {
				await aoxona.transferBetweenPublicKeys(nameId1, account1, account3, account1Balance.add(1).toNumber(), { from: account1 });
				canTransfer = true;
			} catch (e) {
				canTransfer = false;
			}
		...
		it("The AO - transferETH() should be able to transfer ETH to an address", async function() {
			await aoxona.buy({ from: account2, value: new BN(10 ** 5) });

			var canTransferEth;
			try {
				await aoxona.transferEth(recipient.address, new BN(10 ** 4), { from: someAddress });
				canTransferEth = true;
			} catch (e) {
			...
			try {
				await aoxona.transferEth(emptyAddress, new BN(10 ** 4), { from: theAO });
				canTransferEth = true;
			} catch (e) {
				canTransferEth = false;
			}
			...
			try {
				await aoxona.transferEth(recipient.address, new BN(10 ** 8), { from: theAO });
				canTransferEth = true;
			} catch (e) {
				canTransferEth = false;
			}
			...
			try {
				await aoxona.transferEth(recipient.address, new BN(10 ** 4), { from: theAO });
				canTransferEth = true;
			} catch (e) {
				canTransferEth = false;
			}
