...
contract("AOKilo", function(accounts) {
	...
	contract("The AO Only", function() {
		...
		it("The AO - freezeAccount() can freeze account", async function() {
			...
			assert.equal(canFreezeAccount, true, "The AO can't mint AOKilo");
		...
		it("The AO - setPrices() can set prices", async function() {
			...
			assert.notEqual(canSetPrices, true, "Others can set network AOKilo prices");
			try {
			...
			assert.equal(canSetPrices, true, "The AO can't set network AOKilo prices");
	...
	contract("Network Ion Function Tests", function() {
		...
		it("Whitelisted address - mint()  can mint", async function() {
			var canMint;
			try {
				await aokilo.mint(account1, 100, { from: someAddress });
				canMint = true;
			} catch (e) {
				canMint = false;
			}
			...
			try {
				await aokilo.mint(account1, 100, { from: whitelistedAddress });
				canMint = true;
			} catch (e) {
				canMint = false;
			}
		...
		it("Whitelisted address - mintEscrow() should be able to mint and escrow ions to an account", async function() {
			...
			var canMintEscrow;
			try {
				await aokilo.mintEscrow(account1, 10, { from: someAddress });
				canMintEscrow = true;
			} catch (e) {
				canMintEscrow = false;
			}
			...
			try {
				await aokilo.mintEscrow(account1, 10, { from: whitelistedAddress });
				canMintEscrow = true;
			} catch (e) {
				canMintEscrow = false;
			}
		...
		it("buy() - user can buy network ions", async function() {
			...
			await aokilo.mint(aokilo.address, 10 ** 10, { from: whitelistedAddress });
		...
		it("transferBetweenPublicKeys() - should be able to transfer ions between public keys in a Name", async function() {
			...
			try {
				await aokilo.transferBetweenPublicKeys(someAddress, account1, account3, transferAmount, { from: account1 });
				canTransfer = true;
			} catch (e) {
				canTransfer = false;
			}
			...
			try {
				await aokilo.transferBetweenPublicKeys(nameId1, account1, account3, transferAmount, { from: account2 });
				canTransfer = true;
			} catch (e) {
				canTransfer = false;
			}
			...
			try {
				await aokilo.transferBetweenPublicKeys(nameId1, account2, account3, transferAmount, { from: account1 });
				canTransfer = true;
			} catch (e) {
				canTransfer = false;
			}
			...
			try {
				await aokilo.transferBetweenPublicKeys(nameId1, account1, account2, transferAmount, { from: account1 });
				canTransfer = true;
			} catch (e) {
				canTransfer = false;
			}
			...
			var account1Balance = await aokilo.balanceOf(account1);

			try {
				await aokilo.transferBetweenPublicKeys(nameId1, account1, account3, account1Balance.add(1).toNumber(), { from: account1 });
				canTransfer = true;
			} catch (e) {
				canTransfer = false;
			}
			...
			try {
				await aokilo.transferBetweenPublicKeys(nameId1, account1, account3, transferAmount, { from: account1 });
				canTransfer = true;
			} catch (e) {
				canTransfer = false;
			}
			...
			var account1BalanceBefore = await aokilo.balanceOf(account1);
			var account3BalanceBefore = await aokilo.balanceOf(account3);
			...
			var account1BalanceAfter = await aokilo.balanceOf(account1);
			var account3BalanceAfter = await aokilo.balanceOf(account3);
			assert.equal(
				account1BalanceAfter.toNumber(),
		...
		it("The AO - transferETH() should be able to transfer ETH to an address", async function() {
			await aokilo.buy({ from: account2, value: new BN(10 ** 5) });

			var canTransferEth;
			try {
				await aokilo.transferEth(recipient.address, new BN(10 ** 4), { from: someAddress });
				canTransferEth = true;
			} catch (e) {
			...
			try {
				await aokilo.transferEth(emptyAddress, new BN(10 ** 4), { from: theAO });
				canTransferEth = true;
			} catch (e) {
				canTransferEth = false;
			}
			...
			try {
				await aokilo.transferEth(recipient.address, new BN(10 ** 8), { from: theAO });
				canTransferEth = true;
			} catch (e) {
				canTransferEth = false;
			}
			...
			try {
				await aokilo.transferEth(recipient.address, new BN(10 ** 4), { from: theAO });
				canTransferEth = true;
			} catch (e) {
				canTransferEth = false;
			}
