...
contract("AOGiga", function(accounts) {
	var namefactory, taofactory, nametaoposition, logos, aogiga, tokenone, nameId, taoId;
	...
	contract("Variable settings", function() {
		it("should return correct name", async function() {
			var name = await aogiga.name();
			assert.equal(name, "AO Giga", "Contract has the incorrect name");
		});

		it("should return correct symbol", async function() {
			var symbol = await aogiga.symbol();
			assert.equal(symbol, "AOGIGA", "Contract has the incorrect symbol");
		});

		it("should have the correct power of ten", async function() {
			var powerOfTen = await aogiga.powerOfTen();
			assert.equal(powerOfTen, 9, "Contract has the incorrect power of ten");
		});

		it("should have 0 decimal", async function() {
			var decimals = await aogiga.decimals();
			assert.equal(decimals, 9, "Contract has the incorrect decimals");
		});

		it("should have 0 initial supply", async function() {
			var totalSupply = await aogiga.totalSupply();
	...
	contract("The AO Only", function() {
		...
		it("The AO - transferOwnership() - should be able to transfer ownership to a TAO", async function() {
			...
			try {
				await aogiga.transferOwnership(taoId, { from: someAddress });
			...
			try {
				await aogiga.transferOwnership(taoId, { from: theAO });
				canTransferOwnership = true;
			} catch (e) {
				canTransferOwnership = false;
			}
			...
			var newTheAO = await aogiga.theAO();
		...
		it("The AO - setWhitelist() should be able to whitelist an address", async function() {
			var canSetWhitelist;
			try {
				await aogiga.setWhitelist(whitelistedAddress, true, { from: someAddress });
			...
			try {
				await aogiga.setWhitelist(whitelistedAddress, true, { from: account1 });
				canSetWhitelist = true;
			} catch (e) {
				canSetWhitelist = false;
			}
			...
			var whitelistStatus = await aogiga.whitelist(whitelistedAddress);
		...
		it("The AO - setNameTAOPositionAddress() should be able to set NameTAOPosition address", async function() {
			var canSetAddress;
			try {
				await aogiga.setNameTAOPositionAddress(nametaoposition.address, { from: someAddress });
				canSetAddress = true;
			} catch (e) {
				canSetAddress = false;
			}
			...
			try {
				await aogiga.setNameTAOPositionAddress(nametaoposition.address, { from: account1 });
				canSetAddress = true;
			} catch (e) {
				canSetAddress = false;
			}
			...
			var nameTAOPositionAddress = await aogiga.nameTAOPositionAddress();
			assert.equal(nameTAOPositionAddress, nametaoposition.address, "Contract has incorrect nameTAOPositionAddress");
		});

		it("The AO - transferERC20() should be able to transfer ERC20 to an address", async function() {
			await tokenone.transfer(aogiga.address, 100, { from: theAO });

			var accountBalanceBefore = await tokenone.balanceOf(account1);
			var aogigaBalanceBefore = await tokenone.balanceOf(aogiga.address);

			var canTransfer;
			try {
				await aogiga.transferERC20(tokenone.address, account1, 10, { from: someAddress });
				canTransfer = true;
			} catch (e) {
				canTransfer = false;
			}
			assert.equal(canTransfer, false, "Non-AO can transfer ERC20 token from aogiga");

			try {
				await aogiga.transferERC20(tokenone.address, account1, 1000, { from: account1 });
				canTransfer = true;
			} catch (e) {
				canTransfer = false;
			}
			...
			try {
				await aogiga.transferERC20(tokenone.address, account1, 100, { from: account1 });
				canTransfer = true;
			} catch (e) {
				canTransfer = false;
			}
			assert.equal(canTransfer, true, "The AO can't transfer ERC20 token from aogiga to another recipient");

			var accountBalanceAfter = await tokenone.balanceOf(account1);
			var aogigaBalanceAfter = await tokenone.balanceOf(aogiga.address);

			assert.equal(accountBalanceAfter.toNumber(), accountBalanceBefore.plus(100).toNumber(), "Account has incorrect ERC20 balance");
			assert.equal(aogigaBalanceAfter.toNumber(), aogigaBalanceBefore.minus(100).toNumber(), "aogiga has incorrect ERC20 balance");
		});

		it("The AO - freezeAccount() can freeze account", async function() {
			var canFreezeAccount;
			try {
				await aogiga.freezeAccount(account2, true, { from: someAddress });
			...
			try {
				await aogiga.freezeAccount(account2, true, { from: account1 });
				canFreezeAccount = true;
			} catch (e) {
				canFreezeAccount = false;
			}
			assert.equal(canFreezeAccount, true, "The AO can't mint AOGiga");
			var account2Frozen = await aogiga.frozenAccount(account2);
			assert.equal(account2Frozen, true, "Account2 is not frozen after The AO froze his account");

			await aogiga.freezeAccount(account2, false, { from: account1 });
		});

		it("The AO - setPrices() can set prices", async function() {
			var canSetPrices;
			try {
				await aogiga.setPrices(2, 2, { from: someAddress });
				canSetPrices = true;
			} catch (e) {
				canSetPrices = false;
			}
			assert.notEqual(canSetPrices, true, "Others can set network AOGiga prices");
			try {
				await aogiga.setPrices(2, 2, { from: account1 });
				canSetPrices = true;
			} catch (e) {
				canSetPrices = false;
			}
			assert.equal(canSetPrices, true, "The AO can't set network AOGiga prices");
			var sellPrice = await aogiga.sellPrice();
			var buyPrice = await aogiga.buyPrice();
	...
	contract("Network Ion Function Tests", function() {
		before(async function() {
			await aogiga.setWhitelist(whitelistedAddress, true, { from: theAO });
		});

		it("Whitelisted address - mint()  can mint", async function() {
			var canMint;
			try {
				await aogiga.mint(account1, 100, { from: someAddress });
				canMint = true;
			} catch (e) {
				canMint = false;
			}
			...
			var balanceBefore = await aogiga.balanceOf(account1);
			var totalSupplyBefore = await aogiga.totalSupply();
			try {
				await aogiga.mint(account1, 100, { from: whitelistedAddress });
				canMint = true;
			} catch (e) {
				canMint = false;
			}
			...
			var balanceAfter = await aogiga.balanceOf(account1);
			var totalSupplyAfter = await aogiga.totalSupply();
		...
		it("WhitelistedAddress - stakeFrom() should be able to stake ions on behalf of others", async function() {
			...
			var canStake;
			try {
				await aogiga.stakeFrom(account1, 10, { from: someAddress });
				canStake = true;
			} catch (e) {
				canStake = false;
			}
			...
			try {
				await aogiga.stakeFrom(account1, 100000, { from: whitelistedAddress });
				canStake = true;
			} catch (e) {
				canStake = false;
			}
			...
			try {
				await aogiga.stakeFrom(account1, 10, { from: whitelistedAddress });
				canStake = true;
			} catch (e) {
				canStake = false;
			}
		...
		it("Whitelisted address - unstakeFrom() should be able to unstake ions on behalf of others", async function() {
			...
			var canUnstake;
			try {
				await aogiga.unstakeFrom(account1, 10, { from: someAddress });
				canUnstake = true;
			} catch (e) {
				canUnstake = false;
			}
			...
			try {
				await aogiga.unstakeFrom(account1, 100000, { from: whitelistedAddress });
				canUnstake = true;
			} catch (e) {
				canUnstake = false;
			}
			...
			try {
				await aogiga.unstakeFrom(account1, 10, { from: whitelistedAddress });
				canUnstake = true;
			} catch (e) {
				canUnstake = false;
			}
		...
		it("Whitelisted address - escrowFrom() should be able to escrow ions on behalf of others", async function() {
			var account1BalanceBefore = await aogiga.balanceOf(account1);
			var account2BalanceBefore = await aogiga.balanceOf(account2);
			var account2EscrowedBalanceBefore = await aogiga.escrowedBalance(account2);
			var totalSupplyBefore = await aogiga.totalSupply();

			var canEscrow;
			try {
				await aogiga.escrowFrom(account1, account2, 10, { from: someAddress });
				canEscrow = true;
			} catch (e) {
				canEscrow = false;
			}
			...
			try {
				await aogiga.escrowFrom(account1, account2, 1000, { from: whitelistedAddress });
				canEscrow = true;
			} catch (e) {
				canEscrow = false;
			}
			...
			try {
				await aogiga.escrowFrom(account1, account2, 10, { from: whitelistedAddress });
				canEscrow = true;
			} catch (e) {
				canEscrow = false;
			}
			...
			var account1BalanceAfter = await aogiga.balanceOf(account1);
			var account2BalanceAfter = await aogiga.balanceOf(account2);
			var account2EscrowedBalanceAfter = await aogiga.escrowedBalance(account2);
			var totalSupplyAfter = await aogiga.totalSupply();
		...
		it("Whitelisted address - mintEscrow() should be able to mint and escrow ions to an account", async function() {
			var account1BalanceBefore = await aogiga.balanceOf(account1);
			var account1EscrowedBalanceBefore = await aogiga.escrowedBalance(account1);
			var totalSupplyBefore = await aogiga.totalSupply();

			var canMintEscrow;
			try {
				await aogiga.mintEscrow(account1, 10, { from: someAddress });
				canMintEscrow = true;
			} catch (e) {
				canMintEscrow = false;
			}
			...
			try {
				await aogiga.mintEscrow(account1, 10, { from: whitelistedAddress });
				canMintEscrow = true;
			} catch (e) {
				canMintEscrow = false;
			}
			...
			var account1BalanceAfter = await aogiga.balanceOf(account1);
			var account1EscrowedBalanceAfter = await aogiga.escrowedBalance(account1);
			var totalSupplyAfter = await aogiga.totalSupply();
		...
		it("Whitelisted address - unescrowFrom() should be able to unescrow ions for an account", async function() {
			...
			var canUnescrow;
			try {
				await aogiga.unescrowFrom(account1, 10, { from: someAddress });
				canUnescrow = true;
			} catch (e) {
				canUnescrow = false;
			}
			...
			try {
				await aogiga.unescrowFrom(account1, 100000, { from: whitelistedAddress });
				canUnescrow = true;
			} catch (e) {
				canUnescrow = false;
			}
			...
			try {
				await aogiga.unescrowFrom(account1, 10, { from: whitelistedAddress });
				canUnescrow = true;
			} catch (e) {
				canUnescrow = false;
			}
		...
		it("Whitelisted address - whitelistBurnFrom() should be able to burn ions on behalf of others", async function() {
			...
			var canBurn;
			try {
				await aogiga.whitelistBurnFrom(account1, 10, { from: someAddress });
				canBurn = true;
			} catch (e) {
				canBurn = false;
			}
			...
			try {
				await aogiga.whitelistBurnFrom(account1, 1000000, { from: whitelistedAddress });
				canBurn = true;
			} catch (e) {
				canBurn = false;
			}
			...
			try {
				await aogiga.whitelistBurnFrom(account1, 10, { from: whitelistedAddress });
				canBurn = true;
			} catch (e) {
				canBurn = false;
			}
		...
		it("Whitelisted address - whitelistTransferFrom() should be able to transfer ions from an address to another address", async function() {
			...
			var canTransferFrom;
			try {
				await aogiga.whitelistTransferFrom(account1, account2, 10, { from: someAddress });
			...
			try {
				await aogiga.whitelistTransferFrom(account1, account2, 1000000, { from: whitelistedAddress });
				canTransferFrom = true;
			} catch (e) {
				canTransferFrom = false;
			}
			...
			try {
				await aogiga.whitelistTransferFrom(account1, account2, 10, { from: whitelistedAddress });
				canTransferFrom = true;
			} catch (e) {
				canTransferFrom = false;
			}
		...
		it("buy() - user can buy network ions", async function() {
			await aogiga.setPrices(1, 1, { from: theAO });

			var canBuy;
			try {
				await aogiga.buy({ from: account2, value: 10 });
				canBuy = true;
			} catch (e) {
				canBuy = false;
			}
			assert.notEqual(canBuy, true, "Contract does not have enough network ion balance to complete user's ion purchase");
			await aogiga.mint(aogiga.address, 10 ** 20, { from: whitelistedAddress });
		...
		it("sell() - user can sell network ions to contract", async function() {
			await aogiga.setPrices(100, 1, { from: theAO });

			var canSell;
			try {
				await aogiga.sell(10, { from: account2 });
				canSell = true;
			} catch (e) {
				canSell = false;
			}
			...
			var account2BalanceBefore = await aogiga.balanceOf(account2);
			var contractBalanceBefore = await aogiga.balanceOf(aogiga.address);

			try {
				await aogiga.sell(5, { from: account2 });
				canSell = true;
			} catch (e) {
				canSell = false;
			}
			...
			var account2BalanceAfter = await aogiga.balanceOf(account2);
			var contractBalanceAfter = await aogiga.balanceOf(aogiga.address);
			assert.equal(
				account2BalanceAfter.toNumber(),
		...
		it("transfer() - should send correct `_value` to `_to` from your account", async function() {
			var account1BalanceBefore = await aogiga.balanceOf(account1);
			var account2BalanceBefore = await aogiga.balanceOf(account2);
			await aogiga.transfer(account2, 10, { from: account1 });
			account1BalanceAfter = await aogiga.balanceOf(account1);
			account2BalanceAfter = await aogiga.balanceOf(account2);
			assert.equal(
		...
		it("burn() - should remove `_value` ions from the system irreversibly", async function() {
			var account1BalanceBefore = await aogiga.balanceOf(account1);
			await aogiga.burn(10, { from: account1 });
			account1BalanceAfter = await aogiga.balanceOf(account1);
			assert.equal(
				account1BalanceAfter.toNumber(),
				account1BalanceBefore.minus(10).toNumber(),
		...
		it("approve() - should set allowance for other address", async function() {
			var account2AllowanceBefore = await aogiga.allowance(account1, account2);
			await aogiga.approve(account2, 10, { from: account1 });
			var account2AllowanceAfter = await aogiga.allowance(account1, account2);
			assert.equal(
		...
		it("transferFrom() - should send `_value` ions to `_to` in behalf of `_from`", async function() {
			var canTransferFrom;
			try {
				await aogiga.transferFrom(account1, account2, 5, { from: someAddress });
				canTransferFrom = true;
			} catch (e) {
				canTransferFrom = false;
			}
			...
			try {
				await aogiga.transferFrom(account1, account2, 1000, { from: account2 });
				canTransferFrom = true;
			} catch (e) {
				canTransferFrom = false;
			}
			...
			try {
				await aogiga.transferFrom(account1, account2, 5, { from: account2 });
				canTransferFrom = true;
			} catch (e) {
				canTransferFrom = false;
			}
		...
		it("burnFrom() - should remove `_value` ions from the system irreversibly on behalf of `_from`", async function() {
			var canBurnFrom;
			try {
				await aogiga.burnFrom(account1, 5, { from: someAddress });
				canBurnFrom = true;
			} catch (e) {
				canBurnFrom = false;
			}
			...
			try {
				await aogiga.burnFrom(account1, 10, { from: account2 });
				canBurnFrom = true;
			} catch (e) {
				canBurnFrom = false;
			}
			...
			try {
				await aogiga.burnFrom(account1, 5, { from: account2 });
				canBurnFrom = true;
			} catch (e) {
				canBurnFrom = false;
			}
		...
		it("frozen account should NOT be able to transfer", async function() {
			await aogiga.freezeAccount(account1, true, { from: theAO });
			...
			// Unfreeze account1
			await aogiga.freezeAccount(account1, false, { from: theAO });
		});

		it("The AO - transferETH() should be able to transfer ETH to an address", async function() {
			await aogiga.buy({ from: account2, value: web3.toWei(2, "ether") });

			var canTransferEth;
			try {
				await aogiga.transferEth(recipient.address, web3.toWei(1, "ether"), { from: someAddress });
				canTransferEth = true;
			} catch (e) {
			...
			try {
				await aogiga.transferEth(emptyAddress, web3.toWei(1, "ether"), { from: theAO });
				canTransferEth = true;
			} catch (e) {
				canTransferEth = false;
			}
			...
			try {
				await aogiga.transferEth(recipient.address, web3.toWei(1000, "ether"), { from: theAO });
				canTransferEth = true;
			} catch (e) {
				canTransferEth = false;
			}
			...
			try {
				await aogiga.transferEth(recipient.address, web3.toWei(1, "ether"), { from: theAO });
				canTransferEth = true;
			} catch (e) {
				canTransferEth = false;
			}
