...
contract("AOExa", function(accounts) {
	...
	contract("Network Ion Function Tests", function() {
		...
		it("buy() - user can buy network ions", async function() {
			...
			await aoexa.mint(aoexa.address, 10 ** 20, { from: whitelistedAddress });
		...
		it("transferBetweenPublicKeys() - should be able to transfer ions between public keys in a Name", async function() {
			...
			try {
				await aoexa.transferBetweenPublicKeys(nameId1, account1, account3, account1Balance.plus(1).toNumber(), { from: account1 });
				canTransfer = true;
			} catch (e) {
				canTransfer = false;
			}
