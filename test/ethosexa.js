...
contract("Ethos Exa", function(accounts) {
	...
	it("Whitelisted Address - should be able to mint EthosExa to a TAO/Name", async function() {
		var canMint;
		try {
			await ethosexa.mint(nameId1, 1000, { from: someAddress });
			canMint = true;
		} catch (e) {
			canMint = false;
		}
		assert.equal(canMint, false, "Non-whitelisted address can mint EthosExa");

		try {
			await ethosexa.mint(someAddress, 1000, { from: whitelistedAddress });
			canMint = true;
		} catch (e) {
			canMint = false;
		}
		assert.equal(canMint, false, "Whitelisted address can mint EthosExa to non Name/TAO");

		try {
			await ethosexa.mint(nameId1, 1000, { from: whitelistedAddress });
			canMint = true;
		} catch (e) {
			canMint = false;
		}
		assert.equal(canMint, true, "Whitelisted address can't mint EthosExa to Name");
		...
		try {
			await ethosexa.mint(taoId1, 1000, { from: whitelistedAddress });
			canMint = true;
		} catch (e) {
			canMint = false;
		}
		assert.equal(canMint, true, "Whitelisted address can't mint EthosExa to TAO");

		var taoBalance = await ethosexa.balanceOf(taoId1);
		assert.equal(taoBalance.toNumber(), 1000, "TAO has incorrect ethosexa balance");
	});

	it("Whitelisted Address - should be able to transfer EthosExa from a Name/TAO to another Name/TAO", async function() {
		var canTransferFrom;
		try {
		...
		assert.equal(canTransferFrom, false, "Non-whitelisted address can transfer EthosExa");
		...
		assert.equal(canTransferFrom, false, "Whitelisted address can transfer EthosExa to non Name/TAO");
		...
		assert.equal(canTransferFrom, false, "Whitelisted address can transfer EthosExa more than owned balance");
		...
		assert.equal(canTransferFrom, true, "Whitelisted address can't transfer EthosExa from Name to Name");
		...
		assert.equal(canTransferFrom, true, "Whitelisted address can't transfer EthosExa from Name to TAO");
		...
		assert.equal(canTransferFrom, true, "Whitelisted address can't transfer EthosExa from TAO to Name");
		...
		assert.equal(canTransferFrom, true, "Whitelisted address can't transfer EthosExa from TAO to TAO");
	...
	it("Whitelisted Address - should be able to burn EthosExa from a TAO/Name", async function() {
		...
		assert.equal(canWhitelistBurnFrom, false, "Non-whitelisted address can burn EthosExa");
		...
		assert.equal(canWhitelistBurnFrom, false, "Whitelisted address can burn EthosExa from non Name/TAO");
		...
		assert.equal(canWhitelistBurnFrom, false, "Whitelisted address can burn EthosExa more than owned balance");
		...
		assert.equal(canWhitelistBurnFrom, true, "Whitelisted address can't burn EthosExa from Name");
		...
		assert.equal(canWhitelistBurnFrom, true, "Whitelisted address can't burn EthosExa from TAO");
