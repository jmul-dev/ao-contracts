...
contract("AOSettingAttribute", function(accounts) {
	...
	before(async function() {
		...
		// Create a name
		var result = await namefactory.createName("beta", "somedathash", "somedatabase", "somekeyvalue", "somecontentid", { from: account1 });
		creatorThoughtNameId = await namefactory.ethAddressToNameId(account1);

		result = await namefactory.createName("charlie", "somedathash", "somedatabase", "somekeyvalue", "somecontentid", { from: account2 });
		...
		// Create a thought
		result = await thoughtfactory.createThought("somedathash", "somedatabase", "somekeyvalue", "somecontentid", creatorThoughtNameId, {from: account1});
		var createThoughtEvent = result.logs[0];
		creatorThoughtId = createThoughtEvent.args.thoughtId;

		result = await thoughtfactory.createThought("somedathash", "somedatabase", "somekeyvalue", "somecontentid", creatorThoughtNameId, {from: account2});
		createThoughtEvent = result.logs[0];
		associatedThoughtId = createThoughtEvent.args.thoughtId;

		result = await thoughtfactory.createThought("somedathash", "somedatabase", "somekeyvalue", "somecontentid", creatorThoughtNameId, {from: account3});
		createThoughtEvent = result.logs[0];
		proposalThoughtId = createThoughtEvent.args.thoughtId;
	})

	it("only whitelisted address can add setting attribute (Data/State)", async function() {
		var canAdd;
		try {
			var result = await aosettingattribute.add(settingId1, creatorThoughtNameId, settingType, settingName, creatorThoughtId, associatedThoughtId, extraData, {from: account1});
			canAdd = true;
		} catch (e) {
			canAdd = false;
		}
		...
		try {
			var result = await aosettingattribute.add(settingId1, creatorThoughtNameId, settingType, settingName, creatorThoughtId, associatedThoughtId, extraData, {from: whitelistedAccount});
			canAdd = true;
		} catch (e) {
			canAdd = false;
		}
		...
		// Add another setting
		var result = await aosettingattribute.add(settingId2, creatorThoughtNameId, settingType, settingName, creatorThoughtId, associatedThoughtId, extraData, {from: whitelistedAccount});
	});

	it("non-approved setting can not be finalized", async function() {
		var canFinalize;
		try {
		...
		assert.equal(canFinalize, false, "Non-approved setting can be finalized");
	});
