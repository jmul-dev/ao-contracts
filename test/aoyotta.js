...
contract("AOYotta", function(accounts) {
	var namefactory, taofactory, nametaoposition, logos, aoyotta, tokenone, nameId, taoId;
	...
	contract("The AO Only", function() {
		...
		it("The AO - transferERC20() should be able to transfer ERC20 to an address", async function() {
			await tokenone.transfer(aoyotta.address, 100, { from: theAO });

			var accountBalanceBefore = await tokenone.balanceOf(account1);
			var aoyottaBalanceBefore = await tokenone.balanceOf(aoyotta.address);

			var canTransfer;
			try {
				await aoyotta.transferERC20(tokenone.address, account1, 10, { from: someAddress });
				canTransfer = true;
			} catch (e) {
				canTransfer = false;
			}
			assert.equal(canTransfer, false, "Non-AO can transfer ERC20 token from aoyotta");

			try {
				await aoyotta.transferERC20(tokenone.address, account1, 1000, { from: account1 });
				canTransfer = true;
			} catch (e) {
				canTransfer = false;
			}
			...
			try {
				await aoyotta.transferERC20(tokenone.address, account1, 100, { from: account1 });
				canTransfer = true;
			} catch (e) {
				canTransfer = false;
			}
			assert.equal(canTransfer, true, "The AO can't transfer ERC20 token from aoyotta to another recipient");

			var accountBalanceAfter = await tokenone.balanceOf(account1);
			var aoyottaBalanceAfter = await tokenone.balanceOf(aoyotta.address);

			assert.equal(accountBalanceAfter.toNumber(), accountBalanceBefore.plus(100).toNumber(), "Account has incorrect ERC20 balance");
			assert.equal(aoyottaBalanceAfter.toNumber(), aoyottaBalanceBefore.minus(100).toNumber(), "aoyotta has incorrect ERC20 balance");
		});
	...
	contract("Network Token Function Tests", function() {
		...
		it("Whitelisted address - mintToken()  can mint token", async function() {
			var canMint;
			try {
				await aoyotta.mintToken(account1, 100, { from: someAddress });
				canMint = true;
			} catch (e) {
				canMint = false;
			}
			...
			try {
				await aoyotta.mintToken(account1, 100, { from: whitelistedAddress });
				canMint = true;
			} catch (e) {
				canMint = false;
			}
		...
		it("Whitelisted address - mintTokenEscrow() should be able to mint and escrow tokens to an account", async function() {
			...
			var canMintEscrow;
			try {
				await aoyotta.mintTokenEscrow(account1, 10, { from: someAddress });
				canMintEscrow = true;
			} catch (e) {
				canMintEscrow = false;
			}
			...
			try {
				await aoyotta.mintTokenEscrow(account1, 10, { from: whitelistedAddress });
				canMintEscrow = true;
			} catch (e) {
				canMintEscrow = false;
			}
		...
		it("buy() - user can buy network tokens", async function() {
			...
			await aoyotta.mintToken(aoyotta.address, 10 ** 20, { from: whitelistedAddress });
		...
		it("The AO - transferETH() should be able to transfer ETH to an address", async function() {
			await aoyotta.buy({ from: account2, value: web3.toWei(2, "ether") });

			var canTransferEth;
			try {
				await aoyotta.transferEth(recipient.address, web3.toWei(1, "ether"), { from: someAddress });
				canTransferEth = true;
			} catch (e) {
			...
			try {
				await aoyotta.transferEth(emptyAddress, web3.toWei(1, "ether"), { from: theAO });
				canTransferEth = true;
			} catch (e) {
				canTransferEth = false;
			}
			...
			try {
				await aoyotta.transferEth(recipient.address, web3.toWei(1000, "ether"), { from: theAO });
				canTransferEth = true;
			} catch (e) {
				canTransferEth = false;
			}
			...
			try {
				await aoyotta.transferEth(recipient.address, web3.toWei(1, "ether"), { from: theAO });
				canTransferEth = true;
			} catch (e) {
				canTransferEth = false;
			}
