...
contract("AOPeta", function(accounts) {
	var namefactory, taofactory, nametaoposition, logos, aopeta, tokenone, nameId, taoId;
	...
	contract("Variable settings", function() {
		it("should return correct name", async function() {
			var name = await aopeta.name();
			assert.equal(name, "AO Peta", "Contract has the incorrect name");
		});

		it("should return correct symbol", async function() {
			var symbol = await aopeta.symbol();
			assert.equal(symbol, "AOPETA", "Contract has the incorrect symbol");
		});

		it("should have the correct power of ten", async function() {
			var powerOfTen = await aopeta.powerOfTen();
			assert.equal(powerOfTen, 15, "Contract has the incorrect power of ten");
		});

		it("should have 0 decimal", async function() {
			var decimals = await aopeta.decimals();
			assert.equal(decimals, 15, "Contract has the incorrect decimals");
		});

		it("should have 0 initial supply", async function() {
			var totalSupply = await aopeta.totalSupply();
	...
	contract("The AO Only", function() {
		...
		it("The AO - transferOwnership() - should be able to transfer ownership to a TAO", async function() {
			...
			try {
				await aopeta.transferOwnership(taoId, { from: someAddress });
			...
			try {
				await aopeta.transferOwnership(taoId, { from: theAO });
				canTransferOwnership = true;
			} catch (e) {
				canTransferOwnership = false;
			}
			...
			var newTheAO = await aopeta.theAO();
		...
		it("The AO - setWhitelist() should be able to whitelist an address", async function() {
			var canSetWhitelist;
			try {
				await aopeta.setWhitelist(whitelistedAddress, true, { from: someAddress });
			...
			try {
				await aopeta.setWhitelist(whitelistedAddress, true, { from: account1 });
				canSetWhitelist = true;
			} catch (e) {
				canSetWhitelist = false;
			}
			...
			var whitelistStatus = await aopeta.whitelist(whitelistedAddress);
		...
		it("The AO - setNameTAOPositionAddress() should be able to set NameTAOPosition address", async function() {
			var canSetAddress;
			try {
				await aopeta.setNameTAOPositionAddress(nametaoposition.address, { from: someAddress });
				canSetAddress = true;
			} catch (e) {
				canSetAddress = false;
			}
			...
			try {
				await aopeta.setNameTAOPositionAddress(nametaoposition.address, { from: account1 });
				canSetAddress = true;
			} catch (e) {
				canSetAddress = false;
			}
			...
			var nameTAOPositionAddress = await aopeta.nameTAOPositionAddress();
			assert.equal(nameTAOPositionAddress, nametaoposition.address, "Contract has incorrect nameTAOPositionAddress");
		});

		it("The AO - transferERC20() should be able to transfer ERC20 to an address", async function() {
			await tokenone.transfer(aopeta.address, 100, { from: theAO });

			var accountBalanceBefore = await tokenone.balanceOf(account1);
			var aopetaBalanceBefore = await tokenone.balanceOf(aopeta.address);

			var canTransfer;
			try {
				await aopeta.transferERC20(tokenone.address, account1, 10, { from: someAddress });
				canTransfer = true;
			} catch (e) {
				canTransfer = false;
			}
			assert.equal(canTransfer, false, "Non-AO can transfer ERC20 token from aopeta");

			try {
				await aopeta.transferERC20(tokenone.address, account1, 1000, { from: account1 });
				canTransfer = true;
			} catch (e) {
				canTransfer = false;
			}
			...
			try {
				await aopeta.transferERC20(tokenone.address, account1, 100, { from: account1 });
				canTransfer = true;
			} catch (e) {
				canTransfer = false;
			}
			assert.equal(canTransfer, true, "The AO can't transfer ERC20 token from aopeta to another recipient");

			var accountBalanceAfter = await tokenone.balanceOf(account1);
			var aopetaBalanceAfter = await tokenone.balanceOf(aopeta.address);

			assert.equal(accountBalanceAfter.toNumber(), accountBalanceBefore.plus(100).toNumber(), "Account has incorrect ERC20 balance");
			assert.equal(aopetaBalanceAfter.toNumber(), aopetaBalanceBefore.minus(100).toNumber(), "aopeta has incorrect ERC20 balance");
		});

		it("The AO - freezeAccount() can freeze account", async function() {
			var canFreezeAccount;
			try {
				await aopeta.freezeAccount(account2, true, { from: someAddress });
			...
			try {
				await aopeta.freezeAccount(account2, true, { from: account1 });
				canFreezeAccount = true;
			} catch (e) {
				canFreezeAccount = false;
			}
			assert.equal(canFreezeAccount, true, "The AO can't mint AOPeta");
			var account2Frozen = await aopeta.frozenAccount(account2);
			assert.equal(account2Frozen, true, "Account2 is not frozen after The AO froze his account");

			await aopeta.freezeAccount(account2, false, { from: account1 });
		});

		it("The AO - setPrices() can set prices", async function() {
			var canSetPrices;
			try {
				await aopeta.setPrices(2, 2, { from: someAddress });
				canSetPrices = true;
			} catch (e) {
				canSetPrices = false;
			}
			assert.notEqual(canSetPrices, true, "Others can set network AOPeta prices");
			try {
				await aopeta.setPrices(2, 2, { from: account1 });
				canSetPrices = true;
			} catch (e) {
				canSetPrices = false;
			}
			assert.equal(canSetPrices, true, "The AO can't set network AOPeta prices");
			var sellPrice = await aopeta.sellPrice();
			var buyPrice = await aopeta.buyPrice();
	...
	contract("Network Ion Function Tests", function() {
		before(async function() {
			await aopeta.setWhitelist(whitelistedAddress, true, { from: theAO });
		});

		it("Whitelisted address - mint()  can mint", async function() {
			var canMint;
			try {
				await aopeta.mint(account1, 100, { from: someAddress });
				canMint = true;
			} catch (e) {
				canMint = false;
			}
			...
			var balanceBefore = await aopeta.balanceOf(account1);
			var totalSupplyBefore = await aopeta.totalSupply();
			try {
				await aopeta.mint(account1, 100, { from: whitelistedAddress });
				canMint = true;
			} catch (e) {
				canMint = false;
			}
			...
			var balanceAfter = await aopeta.balanceOf(account1);
			var totalSupplyAfter = await aopeta.totalSupply();
		...
		it("WhitelistedAddress - stakeFrom() should be able to stake ions on behalf of others", async function() {
			...
			var canStake;
			try {
				await aopeta.stakeFrom(account1, 10, { from: someAddress });
				canStake = true;
			} catch (e) {
				canStake = false;
			}
			...
			try {
				await aopeta.stakeFrom(account1, 100000, { from: whitelistedAddress });
				canStake = true;
			} catch (e) {
				canStake = false;
			}
			...
			try {
				await aopeta.stakeFrom(account1, 10, { from: whitelistedAddress });
				canStake = true;
			} catch (e) {
				canStake = false;
			}
		...
		it("Whitelisted address - unstakeFrom() should be able to unstake ions on behalf of others", async function() {
			...
			var canUnstake;
			try {
				await aopeta.unstakeFrom(account1, 10, { from: someAddress });
				canUnstake = true;
			} catch (e) {
				canUnstake = false;
			}
			...
			try {
				await aopeta.unstakeFrom(account1, 100000, { from: whitelistedAddress });
				canUnstake = true;
			} catch (e) {
				canUnstake = false;
			}
			...
			try {
				await aopeta.unstakeFrom(account1, 10, { from: whitelistedAddress });
				canUnstake = true;
			} catch (e) {
				canUnstake = false;
			}
		...
		it("Whitelisted address - escrowFrom() should be able to escrow ions on behalf of others", async function() {
			var account1BalanceBefore = await aopeta.balanceOf(account1);
			var account2BalanceBefore = await aopeta.balanceOf(account2);
			var account2EscrowedBalanceBefore = await aopeta.escrowedBalance(account2);
			var totalSupplyBefore = await aopeta.totalSupply();

			var canEscrow;
			try {
				await aopeta.escrowFrom(account1, account2, 10, { from: someAddress });
				canEscrow = true;
			} catch (e) {
				canEscrow = false;
			}
			...
			try {
				await aopeta.escrowFrom(account1, account2, 1000, { from: whitelistedAddress });
				canEscrow = true;
			} catch (e) {
				canEscrow = false;
			}
			...
			try {
				await aopeta.escrowFrom(account1, account2, 10, { from: whitelistedAddress });
				canEscrow = true;
			} catch (e) {
				canEscrow = false;
			}
			...
			var account1BalanceAfter = await aopeta.balanceOf(account1);
			var account2BalanceAfter = await aopeta.balanceOf(account2);
			var account2EscrowedBalanceAfter = await aopeta.escrowedBalance(account2);
			var totalSupplyAfter = await aopeta.totalSupply();
		...
		it("Whitelisted address - mintEscrow() should be able to mint and escrow ions to an account", async function() {
			var account1BalanceBefore = await aopeta.balanceOf(account1);
			var account1EscrowedBalanceBefore = await aopeta.escrowedBalance(account1);
			var totalSupplyBefore = await aopeta.totalSupply();

			var canMintEscrow;
			try {
				await aopeta.mintEscrow(account1, 10, { from: someAddress });
				canMintEscrow = true;
			} catch (e) {
				canMintEscrow = false;
			}
			...
			try {
				await aopeta.mintEscrow(account1, 10, { from: whitelistedAddress });
				canMintEscrow = true;
			} catch (e) {
				canMintEscrow = false;
			}
			...
			var account1BalanceAfter = await aopeta.balanceOf(account1);
			var account1EscrowedBalanceAfter = await aopeta.escrowedBalance(account1);
			var totalSupplyAfter = await aopeta.totalSupply();
		...
		it("Whitelisted address - unescrowFrom() should be able to unescrow ions for an account", async function() {
			...
			var canUnescrow;
			try {
				await aopeta.unescrowFrom(account1, 10, { from: someAddress });
				canUnescrow = true;
			} catch (e) {
				canUnescrow = false;
			}
			...
			try {
				await aopeta.unescrowFrom(account1, 100000, { from: whitelistedAddress });
				canUnescrow = true;
			} catch (e) {
				canUnescrow = false;
			}
			...
			try {
				await aopeta.unescrowFrom(account1, 10, { from: whitelistedAddress });
				canUnescrow = true;
			} catch (e) {
				canUnescrow = false;
			}
		...
		it("Whitelisted address - whitelistBurnFrom() should be able to burn ions on behalf of others", async function() {
			...
			var canBurn;
			try {
				await aopeta.whitelistBurnFrom(account1, 10, { from: someAddress });
				canBurn = true;
			} catch (e) {
				canBurn = false;
			}
			...
			try {
				await aopeta.whitelistBurnFrom(account1, 1000000, { from: whitelistedAddress });
				canBurn = true;
			} catch (e) {
				canBurn = false;
			}
			...
			try {
				await aopeta.whitelistBurnFrom(account1, 10, { from: whitelistedAddress });
				canBurn = true;
			} catch (e) {
				canBurn = false;
			}
		...
		it("Whitelisted address - whitelistTransferFrom() should be able to transfer ions from an address to another address", async function() {
			...
			var canTransferFrom;
			try {
				await aopeta.whitelistTransferFrom(account1, account2, 10, { from: someAddress });
			...
			try {
				await aopeta.whitelistTransferFrom(account1, account2, 1000000, { from: whitelistedAddress });
				canTransferFrom = true;
			} catch (e) {
				canTransferFrom = false;
			}
			...
			try {
				await aopeta.whitelistTransferFrom(account1, account2, 10, { from: whitelistedAddress });
				canTransferFrom = true;
			} catch (e) {
				canTransferFrom = false;
			}
		...
		it("buy() - user can buy network ions", async function() {
			await aopeta.setPrices(1, 1, { from: theAO });

			var canBuy;
			try {
				await aopeta.buy({ from: account2, value: 10 });
				canBuy = true;
			} catch (e) {
				canBuy = false;
			}
			assert.notEqual(canBuy, true, "Contract does not have enough network ion balance to complete user's ion purchase");
			await aopeta.mint(aopeta.address, 10 ** 20, { from: whitelistedAddress });
		...
		it("sell() - user can sell network ions to contract", async function() {
			await aopeta.setPrices(100, 1, { from: theAO });

			var canSell;
			try {
				await aopeta.sell(10, { from: account2 });
				canSell = true;
			} catch (e) {
				canSell = false;
			}
			...
			var account2BalanceBefore = await aopeta.balanceOf(account2);
			var contractBalanceBefore = await aopeta.balanceOf(aopeta.address);

			try {
				await aopeta.sell(5, { from: account2 });
				canSell = true;
			} catch (e) {
				canSell = false;
			}
			...
			var account2BalanceAfter = await aopeta.balanceOf(account2);
			var contractBalanceAfter = await aopeta.balanceOf(aopeta.address);
			assert.equal(
				account2BalanceAfter.toNumber(),
		...
		it("transfer() - should send correct `_value` to `_to` from your account", async function() {
			var account1BalanceBefore = await aopeta.balanceOf(account1);
			var account2BalanceBefore = await aopeta.balanceOf(account2);
			await aopeta.transfer(account2, 10, { from: account1 });
			account1BalanceAfter = await aopeta.balanceOf(account1);
			account2BalanceAfter = await aopeta.balanceOf(account2);
			assert.equal(
		...
		it("burn() - should remove `_value` ions from the system irreversibly", async function() {
			var account1BalanceBefore = await aopeta.balanceOf(account1);
			await aopeta.burn(10, { from: account1 });
			account1BalanceAfter = await aopeta.balanceOf(account1);
			assert.equal(
				account1BalanceAfter.toNumber(),
				account1BalanceBefore.minus(10).toNumber(),
		...
		it("approve() - should set allowance for other address", async function() {
			var account2AllowanceBefore = await aopeta.allowance(account1, account2);
			await aopeta.approve(account2, 10, { from: account1 });
			var account2AllowanceAfter = await aopeta.allowance(account1, account2);
			assert.equal(
		...
		it("transferFrom() - should send `_value` ions to `_to` in behalf of `_from`", async function() {
			var canTransferFrom;
			try {
				await aopeta.transferFrom(account1, account2, 5, { from: someAddress });
				canTransferFrom = true;
			} catch (e) {
				canTransferFrom = false;
			}
			...
			try {
				await aopeta.transferFrom(account1, account2, 1000, { from: account2 });
				canTransferFrom = true;
			} catch (e) {
				canTransferFrom = false;
			}
			...
			try {
				await aopeta.transferFrom(account1, account2, 5, { from: account2 });
				canTransferFrom = true;
			} catch (e) {
				canTransferFrom = false;
			}
		...
		it("burnFrom() - should remove `_value` ions from the system irreversibly on behalf of `_from`", async function() {
			var canBurnFrom;
			try {
				await aopeta.burnFrom(account1, 5, { from: someAddress });
				canBurnFrom = true;
			} catch (e) {
				canBurnFrom = false;
			}
			...
			try {
				await aopeta.burnFrom(account1, 10, { from: account2 });
				canBurnFrom = true;
			} catch (e) {
				canBurnFrom = false;
			}
			...
			try {
				await aopeta.burnFrom(account1, 5, { from: account2 });
				canBurnFrom = true;
			} catch (e) {
				canBurnFrom = false;
			}
		...
		it("frozen account should NOT be able to transfer", async function() {
			await aopeta.freezeAccount(account1, true, { from: theAO });
			...
			// Unfreeze account1
			await aopeta.freezeAccount(account1, false, { from: theAO });
		});

		it("The AO - transferETH() should be able to transfer ETH to an address", async function() {
			await aopeta.buy({ from: account2, value: web3.toWei(2, "ether") });

			var canTransferEth;
			try {
				await aopeta.transferEth(recipient.address, web3.toWei(1, "ether"), { from: someAddress });
				canTransferEth = true;
			} catch (e) {
			...
			try {
				await aopeta.transferEth(emptyAddress, web3.toWei(1, "ether"), { from: theAO });
				canTransferEth = true;
			} catch (e) {
				canTransferEth = false;
			}
			...
			try {
				await aopeta.transferEth(recipient.address, web3.toWei(1000, "ether"), { from: theAO });
				canTransferEth = true;
			} catch (e) {
				canTransferEth = false;
			}
			...
			try {
				await aopeta.transferEth(recipient.address, web3.toWei(1, "ether"), { from: theAO });
				canTransferEth = true;
			} catch (e) {
				canTransferEth = false;
			}
