...
contract("AOPeta", function(accounts) {
	...
	contract("Network Ion Function Tests", function() {
		...
		it("buy() - user can buy network ions", async function() {
			...
			await aopeta.mint(aopeta.address, 10 ** 10, { from: whitelistedAddress });
		...
		it("transferBetweenPublicKeys() - should be able to transfer ions between public keys in a Name", async function() {
			...
			try {
				await aopeta.transferBetweenPublicKeys(nameId1, account1, account3, account1Balance.add(1).toNumber(), { from: account1 });
				canTransfer = true;
			} catch (e) {
				canTransfer = false;
			}
		...
		it("The AO - transferETH() should be able to transfer ETH to an address", async function() {
			await aopeta.buy({ from: account2, value: new BN(10 ** 5) });

			var canTransferEth;
			try {
				await aopeta.transferEth(recipient.address, new BN(10 ** 4), { from: someAddress });
				canTransferEth = true;
			} catch (e) {
			...
			try {
				await aopeta.transferEth(emptyAddress, new BN(10 ** 4), { from: theAO });
				canTransferEth = true;
			} catch (e) {
				canTransferEth = false;
			}
			...
			try {
				await aopeta.transferEth(recipient.address, new BN(10 ** 8), { from: theAO });
				canTransferEth = true;
			} catch (e) {
				canTransferEth = false;
			}
			...
			try {
				await aopeta.transferEth(recipient.address, new BN(10 ** 4), { from: theAO });
				canTransferEth = true;
			} catch (e) {
				canTransferEth = false;
			}
