...
contract("AOPeta", function(accounts) {
	...
	before(function() {
		return AOPeta.deployed().then(function(instance) {
			tokenMeta = instance;
		});
	});
	...
	contract("Variable Setting Tests", function() {
		it("should return correct name", function() {
			return tokenMeta.name.call().then(function(name) {
				assert.equal(name, "AO Peta", "Contract has the wrong name");
			});
		});
		it("should return correct symbol", function() {
			return tokenMeta.symbol.call().then(function(symbol) {
				assert.equal(symbol, "AOPETA", "Contract has the wrong symbol");
			});
		});
		it("should have the correct power of ten", function() {
			return tokenMeta.powerOfTen.call().then(function(powerOfTen) {
				assert.equal(powerOfTen, 15, "Contract has the wrong power of ten");
			});
		});
		it("should have 15 decimals", function() {
			return tokenMeta.decimals.call().then(function(decimals) {
				assert.equal(decimals, 15, "Contract has the wrong decimals");
			});
		});
	...
	contract("ICO Token Function Tests", function() {
		it("owner should NOT be able to set ICO prices", async function() {
			var canSetIcoPrices;
			try {
			...
			assert.equal(canSetIcoPrices, false, "Owner can set ICO token prices");
		});
		it("owner should NOT be able to reserve ICO tokens for the Foundation", async function() {
			...
			assert.equal(canReserveForFoundation, false, "Owner can reserve ICO tokens for the Foundation");
		});
